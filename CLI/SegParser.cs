        ...
        public static S32Data Parse(byte[] data)
        {
            ...
            try
            {
                using (BinaryReader br = new BinaryReader(new MemoryStream(data)))
                {
                    ...
                    // 第二層 - count (WORD) + items (4 bytes each: x, y, ?, ?)
                    if (br.BaseStream.Position + 2 <= br.BaseStream.Length)
                    {
                        int layer2Count = br.ReadUInt16();

                        // 跳過 Layer2 數據 (每項 4 bytes)
                        // SEG 格式的 Layer2 結構不太清楚，暫時跳過
                        long skipBytes = layer2Count * 4;
                        if (br.BaseStream.Position + skipBytes <= br.BaseStream.Length)
                        {
